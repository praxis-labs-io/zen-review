...
- **A block that answers the line above it cannot go to the top row.** A comment hangs under the code it was written against, so topping the card scrolls that code away. Bring its last row on screen and scroll no further up than the line it answers. That is two clamps and no magic number, and it degrades honestly: a card taller than the window keeps the line rather than the card. A box being typed in is the exception and wins outright, because a hunk comment anchors at the top of the hunk and hangs under the bottom of it, and a box off the window holds every key that would scroll to it.
