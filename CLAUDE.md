...
other. A refusal therefore takes its commit back off the ref, so the loser
leaves nothing rather than a generation commit no row names. It gives that up
to a third instance already past it, which is the state a crash there leaves
anyway: the next refresh parents on it and carries on.
