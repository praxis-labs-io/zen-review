...
- **The last block cannot reach the top row unless the pane scrolls past the end.** A key that reads its target off the top row then acts on the block it just left: fold puts a hunk's header up there, and the next press folds a different hunk because the clamp kept the header at the bottom. Let the offset run to the last block's own row.
