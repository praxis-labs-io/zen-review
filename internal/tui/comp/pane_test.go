...
func TestTheFooterIsDrawnAsItWasGiven(t *testing.T) {
	churn := comp.Churn(lipgloss.NewStyle(), testTheme, 630, 105)
...
func TestChurnSaysWhichWayItWent(t *testing.T) {
	got := comp.Churn(lipgloss.NewStyle(), testTheme, 630, 105)

	added := lipgloss.NewStyle().Foreground(testTheme.Success).Render("+630")
	removed := lipgloss.NewStyle().Foreground(testTheme.Error).Render("-105")
